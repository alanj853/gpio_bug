defmodule Reader do
  ...
  def init(_) do
    ...
    {:ok, file} = File.open(output_file, [:write, :append])
    ...
    {:ok, %{can_read: true, output_file: file, pin0_pid: pin0_pid, pin1_pid: pin1_pid}}
  end

  def handle_cast(:stop_read, state = %{output_file: file}) do
    IO.write(file, "GPIO Reading stopped\n")
    {:noreply, %{state | can_read: false}}
  end

  def handle_cast(:start_read, state = %{output_file: file}) do
